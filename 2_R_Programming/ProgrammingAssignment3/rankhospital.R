rankhospital <- function(state, outcome, num) {
  ...
  if (num == "worst") {
    outcome_data[outcome_data[,2]==state & outcome_data$out.rank == max(outcome_data[is.na(outcome_data[,3])==FALSE & outcome_data[,2]==state,4]), 1]
  } else if (num > max(outcome_data[is.na(outcome_data[,3])==FALSE & outcome_data[,2]==state, 4])) {
    NA
  } else {
    outcome_data[outcome_data[,2]==state & outcome_data$out.rank==num, 1]
  }
}